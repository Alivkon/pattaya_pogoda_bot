...
# Попытка получить данные из интернета
    try:
        ...
        if response.status_code == 200:
            ...
            print("Ответ от сервера приливов stormglass.io не 200")
            # Если произошла ошибка при получении данных, записываем сообщение
            prilives = "Об этом я расскажу вам в следующий раз"
    except Exception as e:
        print("Ошибка функции приливов")
        ...
        for item in data_array:
            ...
            if date == current_date:
                height = round(item.get("height", ""),2)
                type_ = item.get("type", "")
                ...
                output_line = f"{formatted_time} {type_} на {height} м.\n"

                # Добавляем строку в текстовую переменную
                prilives += output_line
    return prilives
